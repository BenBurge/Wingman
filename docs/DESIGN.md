...
Toasts are shown by a hidden `powershell.exe -NoProfile -NonInteractive -WindowStyle Hidden` process that loads the WinRT toast APIs and shows the XML `ToastBuilder` builds, so Wingman.Windows never takes a Windows SDK dependency; the AppUserModelID is only valid once `wingman setup` has created the Start Menu shortcut carrying it. Toasts activate through the `wingman:` protocol: the updates-available toast offers "Update all" (`wingman:update-all`) and "Open" (`wingman:updates`), and the batch-finished toast offers "View log" (`wingman:history`); the one exception is the Wingman-updated toast, whose "What's new" action and click open the release page on GitHub. Each kind of toast carries its own tag (`wingman-updates`, `wingman-batch`, `wingman-self-update`), so a later toast of the same kind replaces the last one instead of piling up, giving at most one toast per check. A toast can never fail the command that sent it: a PowerShell failure is written to standard error and otherwise ignored.

`state.json` holds the scheduler's last-known status for the tray to read without running a check of its own: `lastCheck`, `updatesAvailable`, and `updateIds` are written by `wingman check`; `lastBatch`, `lastBatchResult`, and `lastBatchFailed` are written after any batch (`upgrade`, `install`, `import`) finishes; `running` is set for the duration of a check or a batch, and briefly by the tray itself when it starts a check from its menu; `lastError` is set when a check's call to winget fails, or when `check --notify` could not download or start a Wingman update. `lastUpdateCheck`, `latestVersion`, `latestTag`, `latestSetupUrl`, and `latestSha256Url` cache the latest GitHub release for self-update, and `pendingUpdateVersion` names the version `check --notify` started the installer for until that version runs.
...
Self-update reads `https://api.github.com/repos/BenBurge/Wingman/releases/latest` (`GitHubReleaseSource`, parsed with `JsonDocument`) and picks the assets named exactly `wingman-v<version>-<rid>-setup.exe` and `<that name>.sha256`, where the rid is `win-x64` or `win-arm64` from `RuntimeInformation.RuntimeIdentifier`. The answer is cached in `state.json` for six hours, so the TUI's startup check and the scheduled check share one request (GitHub allows 60 unauthenticated requests an hour); `wingman self-update` always asks again and falls back to the cache only when GitHub cannot answer. A 404, a rate limit (403/429), a release without both assets, or a network failure is "no release", never an exception; a tag with a prerelease suffix and a `0.0.0` development build are never offered an update. `UpdateDownloader` reads the hash from the `.sha256` file, streams the installer to `<name>.partial` in `%TEMP%\Wingman\updates` while hashing it, and renames it only when the SHA-256 matches, deleting it and failing otherwise. `SelfUpdateStarter` then runs it with `/VERYSILENT /SUPPRESSMSGBOXES /NORESTART /CURRENTUSER` without waiting; the installer stops the tray and every `wingman.exe` in the install folder, this process included, replaces the executable, runs `wingman setup`, and restarts the tray. A copy counts as installed when its folder is the `InstallLocation` of the installer's uninstall entry (`HKCU\Software\Microsoft\Windows\CurrentVersion\Uninstall\{FC7D592F-76BD-4409-9F32-1C9EAF29A091}_is1`); anything else is portable, which `self-update` only points at the release page unless given `--yes`, and which `check --notify` never updates. With `autoUpdateWingman` on (the default), `check --notify` updates an installed copy on its own: it downloads and verifies the installer, records `pendingUpdateVersion`, and starts it; the first `check --notify` that runs as that version sends a "Wingman updated to v<version>" toast whose "What's new" action opens the release page, unless notifications are paused, and clears the marker. A failed download or hash mismatch is written to `lastError` and never fails the check. In the TUI, the startup check posts "Wingman <version> is available · Settings → Update Wingman", and Settings → Update Wingman asks, downloads with a status line, starts the installer, and quits. Settings → Tools and the `setup`/`self-update` CLI commands share the same `ISetupExecutor` and `ISelfUpdateStarter`.
...
|---|---|---|
| `check` | `--notify` | Lists available upgrades and records the result in `state.json`; `--notify` shows a toast when any are available and, for an installed copy with `autoUpdateWingman` on, installs a newer Wingman release. |
...
| `self-update [--check] [--yes]` | | With `--check`, exits 10 when a newer Wingman is on GitHub Releases, 0 when this one is current, 1 when GitHub could not be reached and nothing is cached; without it, downloads and verifies the newer installer, starts it, and exits 0. A portable copy prints the release page and installs only with `--yes`. Exits 2 off Windows. |
