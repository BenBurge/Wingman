...
The run points `WINGMAN_DATA_DIR` at a temp folder, seeds `package-options.json` there with machine scope and a post-update command for `Microsoft.Azd` so the queue pane shows `⚡ admin`, and deletes the folder at the end, so it never touches the real settings, package options, or history; batches write their history into its `history` folder, which the checks read. Pre- and post-commands go to `FakeCommandRunner`, which prints one line instead of starting a shell, and the elevated helper is never started.
