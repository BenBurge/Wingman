...
for each architecture from the `.sha256` files the release produced. Run it
...
`-Sha256Dir` is a folder containing `wingman-v<version>-win-x64.zip.sha256`
and `wingman-v<version>-win-arm64.zip.sha256`, each one line of the form
