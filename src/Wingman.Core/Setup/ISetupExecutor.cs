...
/// Windows-only; Core only defines the shape so the CLI can run <c>wingman setup</c> through it.
/// </summary>
public interface ISetupExecutor
{
    ...
    /// <see cref="SetupPlanner.Describe(SetupPlan)"/> item in the same order. A failed part does
    /// not stop the others. With <paramref name="dryRun"/> nothing is written and the results say
    /// what would happen.
    /// </summary>
