...
- Setup registers tasks from Task Scheduler XML (`TaskDefinitionXml`) through `schtasks /Create /XML`, written as UTF-16 with a BOM; intervals up to 168 hours, a per-user logon trigger, and laptop-friendly settings all come from the XML. The logon check stays a separate task so its setting can remove it on its own. `TaskDefinitionXml.Matches` decides `unchanged` by comparing command, trigger, and the battery settings against `schtasks /Query /XML`.
- The elevation pipe is created by `SecurePipeServer` with an ACL for the current user and Administrators only; the helper is started with `--parent <pid>` and refuses a server with another id (`GetNamedPipeServerProcessId`). The launcher's rule for the client is `PipePeerCheck.IsHelper`: for `Direct`, the client pid must equal the started process's pid and nothing else is read, since opening a helper elevated under another admin account may be denied; for `PowerShell`, where the helper is the wrapper's child and the pid differs, the client's kernel image path (`QueryFullProcessImageNameW`, compared with this process's own, not `Environment.ProcessPath`, which reports links unresolved) and session must match. The client connects at `TokenImpersonationLevel.Identification`.
- `ElevationSupport.Factory` and `RestartAsAdministrator` read `elevationLauncher` from `settings.json` (honoring `WINGMAN_DATA_DIR`) at each prompt, so the Settings tab only has to save; nothing passes the launcher through the TUI or CLI.
