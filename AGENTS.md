...
- The elevation pipe is created by `SecurePipeServer` with an ACL for the current user and Administrators only; the helper is started with `--parent <pid>` and both ends verify the peer process id (`GetNamedPipeServerProcessId` / `GetNamedPipeClientProcessId`); the client connects at `TokenImpersonationLevel.Identification`.
